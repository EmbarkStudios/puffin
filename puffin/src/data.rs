...
//! The first time it observes an scope it will send detailed information over a separate stream.
//! The profiler consumers, or also sinks, will parse and use the data on the fly.
//!
//! The data format for the first time a scope is seen is as follows:
//!
//! ```ignore
//!    '['          byte       Sentinel
//!    time_ns      i64        Time stamp of when scope started
//!    scope_name   str        Scope name. Human readable, e.g. a function name. Never the empty string.
//!    file_name    str        File name in which the scope is located. Could be the empty string for manually inserted scopes.
//!    line_nmr     str        Line number at which the scope is located. Could be the empty string for manual inserted scopes.
//!    ']'          byte
//! ```
//!
...
const SCOPE_DETAILS_BEGIN: u8 = b'[';
const SCOPE_DETAILS_END: u8 = b']';
...
impl Stream {
    ...
    pub fn write_scope_info(
        ...
        self.0.push(SCOPE_DETAILS_BEGIN);

        self.write_scope_id(scope_id);
        self.write_str(raw_scope_name);
        self.write_str(raw_file_name);
        self.0.write_u32::<LE>(line_nr).expect("can't fail");

        self.0.push(SCOPE_DETAILS_END);
    }
    ...
    pub fn end_scope(&mut self, start_offset: usize, stop_ns: NanoSecond) {
        ...
        // todo: Perhaps use varint encoding
        dest_range
...
// Differentiates the record read from the `Stream`.
pub enum Record<'a> {
    Scope(Scope<'a>),
    ScopeDetails((ScopeId, ScopeDetailsRef<'a>)),
}
...
impl<'s> Reader<'s> {
    ...
    fn parse_scope(&mut self) -> Result<Option<Record<'s>>> {
        match self.peek_u8() {
            Some(SCOPE_DETAILS_BEGIN) => {
                return Ok(Some(Record::ScopeDetails(self.parse_scope_details()?)));
            }
            Some(SCOPE_BEGIN) => {
                return Ok(Some(Record::Scope(self.parse_scope_time()?)));
            }
            Some(_) | None => Ok(None),
        }
    }

    fn parse_scope_details(&mut self) -> Result<(ScopeId, ScopeDetailsRef<'s>)> {
        self.parse_u8()
            .expect("swallowing already peeked SCOPE_INFO_BEGIN");

        let scope_id = self.parse_scope_id()?;
        let scope_name = self.parse_string()?;
        let file_name = self.parse_string()?;
        let line_nr = self.parse_u32()?;

        if self.parse_u8()? != SCOPE_DETAILS_END {
    ...
    fn parse_scope_time(&mut self) -> Result<Scope<'s>> {
        self.parse_u8()
        ...
        Ok(Scope {
            scope_id,
            scope_dynamic_data: ScopeDynamicData {
                start_ns,
    ...
    pub fn read_top_scopes(self) -> Result<Vec<Scope<'s>>> {
        let mut scopes = vec![];
        for scope in self {
            if let Record::Scope(info) = scope? {
                scopes.push(info);
            }
        }
        Ok(scopes)
    ...
    /// Read all the new scopes that were observed this frame.
    /// This function is not exposed publicly because the scope details should be requested via `GlobalProfiler::scope_details`.
    pub(crate) fn read_new_scopes(self) -> Result<Vec<(ScopeId, ScopeDetailsRef<'s>)>> {
        let mut scopes = vec![];
        for scope in self {
            if let Record::ScopeDetails(details) = scope? {
                scopes.push(details);
            }
        }
        Ok(scopes)
    ...
    fn parse_u32(&mut self) -> Result<u32> {
        self.0.read_u32::<LE>().map_err(|_err| Error::PrematureEnd)
    }
    ...
    fn count_all_scopes_at_offset(
        ...
        *max_depth = (*max_depth).max(depth);
        ...
        for child_scope in (Reader::with_offset(stream, offset)?).flatten() {
            if let Record::Scope(scope) = child_scope {
...
fn write_scope_info() {
    ...
    stream.write_scope_info(ScopeId(0), "top", "top.rs", 0);
    let new_scopes = Reader::from_start(&stream).read_new_scopes().unwrap();
...
fn test_profile_data() {
    let stream = {
        let mut stream = Stream::default();
        stream.write_scope_info(ScopeId(0), "top", "top.rs", 0);
        stream.write_scope_info(ScopeId(1), "middle_0", "middle.rs", 0);
        stream.write_scope_info(ScopeId(2), "middle_1", "middle.rs", 42);
    ...
    assert_eq!(
        top_scopes[0].scope_dynamic_data,
        ScopeDynamicData {
            start_ns: 100,
            duration_ns: 300,
            data: "data_top"
    ...
    assert_eq!(
        middle_scopes[0].scope_dynamic_data,
        ScopeDynamicData {
            start_ns: 200,
            duration_ns: 100,
            data: "data_middle_0"
    ...
    assert_eq!(
        middle_scopes[1].scope_dynamic_data,
        ScopeDynamicData {
            start_ns: 300,
            duration_ns: 100,
            data: "data_middle_1"
